...
namespace CalRecycleLCA.Services
{
    public class FragmentLCIAComputation : IFragmentLCIAComputation
    {
        ...
        public void FragmentLCIACompute(int fragmentId, int scenarioId)
        {
            ...
            var scoreCache = _scoreCacheService.Queryable()
                .Join(_nodeCacheService.Queryable().Where(x => x.ScenarioID == scenarioId), sc => sc.NodeCacheID, nc => nc.NodeCacheID, (sc, nc) => new { sc, nc })
                .Join(_fragmentFlowService.Queryable().Where(x => x.FragmentID == fragmentId), nc => nc.nc.FragmentFlowID, ff => ff.FragmentFlowID, (nc, ff) => new { nc, ff });
        ...
        public IEnumerable<FragmentLCIAModel> ComputeFragmentLCIA(int? fragmentId, int? scenarioId, int? lciaMethodId)
        {
            ...
            var lcia = _fragmentFlowService.Queryable()
                ...
                .Join(_nodeCacheService.Queryable().Where(x => x.ScenarioID == scenarioId), ff => ff.FragmentFlowID, nc => nc.FragmentFlowID, (ff, nc) => new { ff, nc })
                .GroupJoin(_scoreCacheService.Queryable()
      ...
      , (s, scoreCaches) => new FragmentLCIAModel
      {
        ...
        public IEnumerable<FragmentLCIAModel> FragmentFlowLCIA(int? fragmentId, int scenarioId, IEnumerable<LCIAMethod> lciaMethods)
        {
            ...
            {
                var nodeCache = _nodeCacheService.Queryable()
                    .Where(x => x.FragmentFlowID == item.ff.FragmentFlowID && x.ScenarioID == scenarioId).ToList();
